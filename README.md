...
- **💾 Sistema de Regras**: Regras automáticas que aplicam classificações e significados no frontend baseadas em padrões de descrição
- **Aplicação Automática**: As regras são aplicadas automaticamente na visualização, sem modificar os arquivos de extração
...
│   ├── rules.get.ts          # Listar regras
│   ├── rules.post.ts         # Salvar regras
...
│   ├── rules.json            # Regras para classificação automática
...
- **💾 Preferências do Sistema**: O sistema agora lembra automaticamente suas configurações de filtro e período selecionado
...
4. **Criar Regra**: Clique em "💾 Criar Regra"
5. **Aplicação Automática**: A partir de agora, transações similares serão classificadas automaticamente
6. **Indicadores**: Transações aplicadas via regras mostram o ícone 💾 na coluna de classificação
...
- **Indicadores Visuais**: 
  - 🧠 Ícone azul na descrição das transações que têm regras aplicadas
...
- **Armazenamento**: Regras salvas em `storage/rules.json`
- **Preservação de Dados**: Os arquivos de extração originais nunca são modificados pelas regras
- **Exemplos de Uso**:
